...
siguiendo exactamente la arquitectura documentada en `CONTEXT_REPORTES_DASHBOARD.md`
...
1. El HTML debe incluir la botonera compartida:
   ...
   documentados en la sección 4 de `CONTEXT_REPORTES_DASHBOARD.md`.
...
3. **Regla del tooltip (obligatoria):** dentro de `downloadPDF`/`downloadWord`
   ...
   como quedó documentado en la sección 10 de `CONTEXT_REPORTES_DASHBOARD.md`.
...
7. Al final, decime si hace falta algún ajuste en `build-index.js` o `index.json`
   ...
   y agregá una fila nueva a la tabla de la sección 6 de `CONTEXT_REPORTES_DASHBOARD.md`
   comparando este informe con los existentes.

**Adjunto:**
1. `CONTEXT_REPORTES_DASHBOARD.md` (contexto del proyecto)
...
- Guardá siempre la versión más reciente de `CONTEXT_REPORTES_DASHBOARD.md` para adjuntar
